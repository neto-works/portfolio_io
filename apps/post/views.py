...
def search(request):
    ...
    p = Post.objects.filter(titulo__icontains=search_term).order_by("-created_at")
    categorias = Categoria.objects.all()
    posts = []
    for post in p:
        post_modificado = {
    ...
    posts_paginados = paginator.get_page(pagina_atual)
    pagination_range = make_pagination_range(paginator.page_range, 10, pagina_atual)
    return render(request,"posts/blog.html",context={"posts_paginados": posts_paginados,"links_pages": pagination_range,"categorias": categorias})
